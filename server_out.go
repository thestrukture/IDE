...
func apiAttempt(w http.ResponseWriter, r *http.Request) (callmet bool) {
	...
	if strings.Contains(r.URL.Path, "/api/get") {
		...
		if r.FormValue("type") == "0" {
			...
			response = NetbDocker(Dex{Misc: r.FormValue("space")})
		}
	...
	if r.Method == "RESET" {
		...
		if strat == "Fast" {
			dockerfile = fmt.Sprintf(dockerLarge, imageName, port, port)
		} else {
			dockerfile = fmt.Sprintf(dockerSmall, imageName, port, port)
		}
...
func bDocker(d Dex) string {
	return NetbDocker(d)
}

//
func NetbDocker(d Dex) string {
	localid := templateIDDocker
	defer templateFNDocker(localid, d)
	output := new(bytes.Buffer)
...
func NetcDocker(args ...interface{}) (d Dex) {
	if len(args) > 0 {
		var jsonBlob = []byte(args[0].(string))
		err := json.Unmarshal(jsonBlob, &d)
		if err != nil {
...
func cDocker(args ...interface{}) (d Dex) {
	if len(args) > 0 {
		d = NetcDocker(args[0])
	} else {
		d = NetcDocker()

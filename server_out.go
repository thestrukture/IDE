...
func net_PluginList(args ...interface{}) string {
	...
	defer func() {
		if n := recover(); n != nil {
			color.Red("Error loading template in path (PluginList) : " + filename)
			// fmt.Println(n)
			DebugTemplatePath(filename, &d)
			//http.Redirect(w,r,"",307)
		}
